...
public class SchedulerUtils {
    ...
    protected static Logger logger = LoggerFactory.getLogger(SchedulerUtils.class);
    ...
    public static JobDetail assemblyJobDetail(JobDetailProxyBean jobDetail){
        ...
        if(StringUtils.isNotBlank(key)){
            ...
            jobBuilder = jobBuilder.withIdentity(String.valueOf(UUID.fromString("JOB")), jobDetail.getGroup());
        }
    ...
    public static CronTrigger assemblyCronTrigger(CronTriggerBean cronTriggerBean){
        ...
        if(StringUtils.isNotBlank(key)){
            ...
            triggerBuilder = triggerBuilder.withIdentity(String.valueOf(UUID.fromString("CronTrigger")), cronTriggerBean.getGroup());
        }
        //设置描述
        ...
        if(StringUtils.isNotBlank(cronTriggerBean.getStartTime())){
            triggerBuilder = triggerBuilder.startAt(StringUtil.toDate(cronTriggerBean.getStartTime(), "yyyy-MM-dd HH:mm:ss"));
        }else{
            triggerBuilder = triggerBuilder.startNow();      //当启动时间为空默认立即启动调度器
        }
        //设置结束时间
        if(StringUtils.isNotBlank(cronTriggerBean.getEndTime())){
            triggerBuilder = triggerBuilder.endAt(StringUtil.toDate(cronTriggerBean.getEndTime(), "yyyy-MM-dd HH:mm:ss"));
        }
        //设置优先级
    ...
    public static SimpleTrigger assemblySimpleTrigger(SimpleTriggerBean simpleTriggerBean){
        ...
        if(StringUtils.isNotBlank(key)){
            ...
            triggerBuilder = triggerBuilder.withIdentity(String.valueOf(UUID.fromString("SimpleTrigger")), simpleTriggerBean.getGroup());
        }
        //设置描述
        ...
        if(StringUtils.isNotBlank(simpleTriggerBean.getStartTime())){
            triggerBuilder = triggerBuilder.startAt(StringUtil.toDate(simpleTriggerBean.getStartTime(), "yyyy-MM-dd HH:mm:ss"));
        }else{
            triggerBuilder = triggerBuilder.startNow();      //当启动时间为空默认立即启动调度器
        }
        //设置结束时间
        if(StringUtils.isNotBlank(simpleTriggerBean.getEndTime())){
            triggerBuilder = triggerBuilder.endAt(StringUtil.toDate(simpleTriggerBean.getEndTime(), "yyyy-MM-dd HH:mm:ss"));
        }
        //设置优先级
